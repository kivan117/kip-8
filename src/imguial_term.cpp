...
ImGuiAl::Log::Log(void* const buffer, size_t const buffer_size)
    ...
    setColor(Level::Trace, CGA::Gray);
    setColor(Level::Debug, CGA::Magenta);
    setColor(Level::Info, CGA::Green);
    setColor(Level::Warning, CGA::Yellow);
    setColor(Level::Error, CGA::Red);
}

void ImGuiAl::Log::trace(char const* const format, ...) {
    va_list args;
    va_start(args, format);
    trace(format, args);
    va_end(args);
...
void ImGuiAl::Log::trace(char const* const format, va_list args) {
    setForegroundColor(_traceTextColor);
...
int ImGuiAl::Log::draw(ImVec2 const& size) {
    ...
    if ((_filterHeaderLabel != nullptr && ImGui::CollapsingHeader(_filterHeaderLabel))) {
        ImGui::PushStyleColor(ImGuiCol_Button, _traceButtonColor);
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, _traceButtonHoveredColor);
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, _traceTextColor);
        bool ok = ImGui::Button(_traceLabel);
...
void ImGuiAl::Log::setColor(Level const level, ImU32 const color) {
    ...
    switch (level) {
        case Level::Trace:
            _traceTextColor = color;
            _traceButtonColor = button_color;
            _traceButtonHoveredColor = hovered_color;
            break;
